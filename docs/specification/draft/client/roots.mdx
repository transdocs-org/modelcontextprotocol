...
模型上下文协议 (MCP) 为客户端以标准方式向服务器暴露文件系统"根目录"提供了机制。根目录定义了服务器在文件系统中可操作的边界，使其能够了解可访问的目录和文件。服务器可以从支持该功能的客户端请求根目录列表，并在其列表发生变化时接收通知。
...
但是，实现可以通过任何适合其需求的界面模式暴露根目录——协议本身不强制要求特定的用户交互模型。
...
`listChanged` 表示当根目录列表发生变化时，客户端是否会发送通知。
