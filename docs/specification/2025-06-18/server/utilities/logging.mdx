...
模型上下文协议（MCP）为服务器向客户端发送结构化日志消息提供了标准化的方式。客户端可以通过设置最低日志级别来控制日志的详细程度，服务器将发送包含严重级别、可选的记录器名称以及任意可序列化为JSON的数据的通知。

## 用户交互模型

实现可以通过任何适合其需求的接口模式来暴露日志功能——协议本身不限定任何特定的用户交互模型。
...
sequenceDiagram
    ...
    服务器-->>客户端: 空结果
    Note over 服务器: 只发送错误级别<br/>及以上的消息
```
