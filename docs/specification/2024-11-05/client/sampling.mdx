...
模型上下文协议（MCP）为服务器通过客户端从语言模型请求LLM采样（“补全”或“生成”）提供了标准化方式。此流程允许客户端控制模型的访问、选择和权限，同时使服务器能够利用AI能力——无需服务器API密钥。
服务器可以请求文本或基于图像的交互，并可选地在其提示中包含来自MCP服务器的上下文。

## 用户交互模型

MCP中的采样允许服务器实现代理行为，通过在其他MCP服务器功能内部嵌套LLM调用。

实现可以自由地通过任何适合其需求的界面模式暴露采样功能——协议本身不规定任何特定的用户交互模型。

<Warning>

出于信任、安全和保障目的，**必须**始终有具备拒绝采样请求能力的人类参与其中。
...
MCP中的模型选择需要仔细抽象，因为服务器和客户端可能使用具有不同模型产品的不同AI提供商。服务器不能简单地按名称请求特定模型，因为客户端可能无法访问该精确模型，或者可能更愿意使用其他提供商的等效模型。

为了解决这个问题，MCP实现了一个偏好系统，结合了抽象的能力优先级和可选的模型提示：

#### 能力优先级

服务器通过三个标准化的优先级值（0-1）表达其需求：

- `costPriority`：成本最小化有多重要？较高值偏好更便宜的模型。
- `speedPriority`：低延迟有多重要？较高值偏好更快的模型。
- `intelligencePriority`：高级功能有多重要？较高值偏好更有能力的模型。
...
客户端处理这些偏好，从其可用选项中选择适当的模型。例如，如果客户端无法访问Claude模型但有Gemini，它可以根据类似能力将sonnet提示映射到`gemini-1.5-pro`。
